...
%%%   - on_tool_call: 每次 tool 调用前触发（通过 pre_invocation filter 注入）
%%%   - on_token: streaming 模式下每收到一个 token 时触发
%%%
%%% 设计原则：
%%%   - 所有回调均为纯观察性用途，不影响执行流程
%%%   - 回调异常时静默忽略，确保不中断主流程
%%%   - 回调未注册时不做任何操作
%%%   - on_llm_call 和 on_tool_call 通过 kernel filter 机制注入，
%%%     无需 agent 自己在 tool loop 中显式调用
%%%
%%% @end
%%%-------------------------------------------------------------------
...
-type callbacks() :: #{
    ...
    on_tool_call   => fun((binary(), map()) -> ok), %% 参数: 函数名, 调用参数
